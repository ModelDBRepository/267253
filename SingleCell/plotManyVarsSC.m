...
showEc = 1; %boolean, set to 1 if you want print mean episode characteristics
...
showpar = 0; %boolean, set to 1 if you want parameter in title
parname = 'g_h'; %Name of parameter to show in the title
parunits = 'nS'; %Units of parameter to show in the title
parcan = 0.34; %set to initial parameter value in sweep
...
xmin = 15; %set the time range you want to plot
xmax = 75;
...
for run4=run4i:run4f
    ...
    for run6=run6i:run6f   %Concatenating portions of time saved in separate files    
        ...
        f1=fopen(VarN);
        yy1=fread(f1,[10, 10000000],'double')';
        fclose(f1);
    ...
    print(f,['ManyVars' dir3 '_' num2str(run6) mrk mrkr '.jpg'],'-djpeg','-r0');
    %close(f);
end